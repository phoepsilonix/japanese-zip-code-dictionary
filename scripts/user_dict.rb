...
file.each_slice(SLICE_NUM) do |rows|
  ...
  results = Parallel.map(rows, in_threads: THREAD_NUM) do | row |
    ...
    # 読みがひらがな,郵便番号以外を含む場合はスキップ => 検証
    next if /[\p{hiragana}\p{katakana}0-9\-]/ !~ yomi
    # 名詞以外の場合はスキップ => しない
    ...
    # 固有名詞は受け入れる
    next if (!$opts[:English] && base =~ /^[a-zA-Z ]+$/ && !ID_DEF[id.to_i].include?("固有名詞") )
    ...
    if ALREADY[generic_expr]
      ...
      hinshi = ID_DEF[id.to_i]
      #line_expr = [ yomi, lcxid, rcxid , mozc_cost, base, hinshi ]
      line_expr = [ yomi, base, hinshi, "" ]
    end
  end
  results.map{ |x|
    next if x.nil?
